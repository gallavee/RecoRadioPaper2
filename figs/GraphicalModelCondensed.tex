\begin{figure}[H]
	\begin{center}
		...
		\node[darkstyle]{\textbf{w}} child[->] foreach \g in {1}
		{node[darkstyle] {$\textbf{w}^{(g)}_{\g}$} child[->] foreach \t [evaluate={\s=int(\g*2+\t-1);}] in {0,1}
			{ node[darkstyle] {$\textbf{w}^{(s)}_{\s}$} child[->] foreach \q [evaluate={\a=int(\s*2+\q-1);}] in {0,1}
				{node[darkstyle] (a\a) {$\textbf{w}^{(a)}_{\a}$} }}} child[->]{node{$\dots$}};
		\foreach \u in {1,...,5}
		{\pgfmathtruncatemacro{\label}{\u}
			\node[darkstyle] (u\u) at (1,-2-\u){$\textbf{w}^{(u)}_\label$};}
		
		\node[plate] (p) at (-2,-7){} ;
		\node[obsnode](r) at (-2,-7){$r_{i}$};
