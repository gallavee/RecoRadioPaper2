...
\end{align}
%%
where  $\sigma(x) \defined (1 + \mathrm{e}^{-x})^{-1}$ denotes the logistic function. Note how the likelihood depends on both per-user and per-artist parameters. The user parameters allow for personalization by deviating according a user specific data. %As we show next, ``cold'' artists are modeled using a hierarchy of informative priors based on the domain taxonomy. 
